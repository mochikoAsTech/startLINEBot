...
この段落内で、注釈でも、このURLを案内すると良さそう https://www.linebiz.com/jp/column/technique/20191128/ 
     
3つめは内容に応じた自動応答メッセージを用意しておいて自動で返信する、という方法です。

Official Account Managerや管理アプリには、応答メッセージとAI応答メッセージというものがあります。応答メッセージはキーワードとメッセージを事前に設定しておくことで、ユーザーがそのキーワードと完全一致するメッセージを送ってきたら、自動でメッセージを返信する機能です。完全に一致する必要があるので、たとえば「メニュー」というキーワードの場合、ユーザーが「メニュー」とだけ送ってくれば応答できますが、「メニューは？」と送ってきた場合は応答できません。
...
これ以降も含め、LINEプラットフォームというワードが適切か確認。（固有名詞のような表現は不必要なら避けたい）
...
このコラム、A/B/Cさんと登場人物を出す必要がない内容に思えましたｗ少なくとも辞めるだれかと、そうじゃない誰か二人で良いし、辞める誰かが基本的に話の真ん中だし
