...
また応答メッセージを送る場合、応答トークンはWebhookを受信後、速やか（具体的には1分以内）に使う必要があります。@<fn>{reply_token_rule}Webhook受信後、すぐに返信を送れる場合は応答メッセージで構いませんが、色々な処理を行ってかなり時間が経ってから返信したい場合は、応答トークンが既に無効になっている可能性があります。その場合は、対象となる友だちのユーザーIDを指定したプッシュメッセージで返信を送ることが可能です。
