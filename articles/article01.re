...
実は、話しかけて自動応答するLINE Botを作るには、話しかける相手として「LINE公式アカウント」というものを用意する必要があります。「いま使っているLINEのアカウントをそのままLINE Botにできないの？」と思われるかもしれませんが、個人のLINEアカウントを操作するためのAPIは提供されていないため、必ず「LINE公式アカウント」が必要なのです。そのため@<chap>{article01}では、このLINE公式アカウントの作り方や、LINE公式アカウントに関する最低限の知識を説明していきます。
