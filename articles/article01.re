...
LINE公式アカウントには、企業や店舗だけでなく、芸能人のものも存在します。たとえば地震のときに誰よりも早く「大丈夫？」とメッセージをくれて、彼氏感がすごい、と話題になった芸能人のLINE公式アカウント@<fn>{takeru}もありました。あまりのプライベートっぽさに、まるで普通にLINEで友だちになったような感覚になりますが、落ち着いてください。あれはLINE公式アカウントです。
...
VOOMとやらがなんなのかということはいまは考えずに進みましょう。きっとショート動画のプラットフォーム的な何かです。VOOMを開くといきなりショート動画が再生されてびびりますが、すかさず［フォロー中］を開いて動画を止め、さらに右上にある人型のアイコンをタップします。（@<img>{select-icon}）
...
//image[mail-address-check-2][4桁の認証番号をLINEで入力][scale=0.6]{
//}

LINEで［メールアドレスが登録されました。］というメッセージが届いたら、メールアドレスの登録は完了です。（@<img>{mail-address-regist-done}）@<fn>{mail}

//image[mail-address-regist-done][メールアドレスを登録するとメッセージが届く][scale=0.6]{
//}
...
まずはLINE公式アカウントのプロフィール画像とアカウント名を設定します。アカウント名と言われてもピンとこないかもしれませんが、もしあなたが技術書典に出展しているサークルさんなら、サークルの名前とアイコンを設定して、サークルの宣伝をするLINE公式アカウントにしてもいいでしょう。あるいは自分のハンドルネームにしてもいいです。このアカウント名はユニークではないため、既存のLINE公式アカウントとかぶる「テストアカウント」のような名称であっても付けられます。動かしたときにうれしくなるので、何かしら納得のいく名前にしておく方がお勧めですが、プロフィール画像もアカウント名も後から変更が可能ですので、悩みすぎずに取りあえずの画像や名前を設定しても構いません。（@<img>{setting-oa-1}）

//image[setting-oa-1][プロフィール画像とアカウント名を設定しよう][scale=0.4]{
//}

カテゴリーの大業種は［個人］を選ぶと、小業種に［個人（芸術・創造・執筆）］があります。たとえば自分の本の告知用という位置づけで作るなら、この［個人（芸術・創造・執筆）］という小業種がよさそうですが、他にも［個人（IT・コンピュータ）］や［個人（学生）］、［個人（その他）］などもあるので、個人的に開発を試す用途など、LINE公式アカウントの使い道に応じて適していると思われる業種を選択してください。［会社・事業者の所在国・地域］は、個人の場合は店舗や居住地を選択すればよいので、もしあなたが日本にお住まいなら［日本］を選んでください。（@<img>{setting-oa-2}）
...
すると、あなたがLINEで登録しているプロフィール情報をLINE公式アカウントの管理アプリに提供して良いか、という認証画面が表示されます。問題なければ［許可する］をタップします。（@<img>{lineoa-app-3}）

//image[lineoa-app-3][［許可する］をタップ][scale=0.4]{
//}

もしここで許可した情報提供を、後で取り消したくなった場合は、LINEの［設定］から［アカウント］を開いて、［連動アプリ］で確認したり、取り消したりできます。
...
//image[target-is-0][自分にはメッセージは届いていなかった][scale=0.8]{
//}
...
結論から言うと、個人のLINEアカウントをLINE公式アカウントに昇格させることはできません。折角個人のLINEアカウントでたくさんの友だちを集めても、それをそのままLINE公式アカウントに引き継ぐことはできないので、仕事や宣伝を目的としてLINEで友だちを増やすなら、最初からLINE公式アカウントではじめるのがオススメです。
...
先ほど作ったLINE公式アカウントの友だち追加画面が表示されました。［友だち 0］［フォロワー 1］と書いてあります。このフォロワーというのはLINE VOOMのフォロワーのことで、さっきLINE公式アカウントを作ったときに、自分で自分をフォローしていたので、この1は自分のことです。知らない誰かではありません。
...
設定した覚えもないのに「はじめまして！友だち追加ありがとうございます」という元気なメッセージが届きました。誰だお前、私が作ったLINE公式アカウントだけど誰だ。後述しますが、これはLINE公式アカウントの「あいさつメッセージ」という機能で、友だち追加されたときに、自動で任意のメッセージを送ることができます。（@<img>{add-friends-4}）

//image[add-friends-4][「あいさつメッセージ」が届いた][scale=0.6]{
//}
...
//image[target-reach][ターゲットリーチが0から1になった][scale=0.8]{
//}

ターゲットリーチとは、要は「送信対象となる友だち」の母数のことのようです。友だちできた…ひとりできた…自分だけど…。鏡に向かって「私たち今日からお友だちだね…！」と言っている気分になってきましたが、例え相手が自分自身だろうが友だちは友だちです！メッセージを送ってみましょう。先ほどと同じように［メッセージを配信する］をタップします。
...
LINE公式アカウントには追加、投稿のボタンが並んでいますが、普通のLINEアカウントだと追加、ブロック、通報のボタンが並んでいます。またLINE公式アカウントの方は、ベーシックIDが表示されていたり、［ほかの公式アカウントを見る］や［友だち限定の情報をゲット］とあるので、よく見れば「これ、LINE公式アカウントだな？」と気づけそうです。
...
認証済アカウントになると、バッジの色が灰色から青色になってついて「公式っぽさ」が増すと共に、LINE内でアカウント名や概要が検索対象@<fn>{account-name}になり、検索結果に表示されるようになります。@<fn>{search}逆に言えば、デフォルトの未認証アカウントであれば、友だち追加のQRコードや、ベーシックIDを自ら流出させない限り、知らない人が勝手に友だち追加される可能性は低いということです。
