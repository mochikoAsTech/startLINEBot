...
続いて［ZHDグループへの情報提供に関する個別規約への同意について］です。色んな同意が求められますが、きちんと読んでから、同意できる内容であれば［同意する］をタップします。（@<img>{zhd}）
...
1と2ならGUIで簡単に送れるけれど、細かい部分で自由度が低いです。一方で3だと自分でコードを書かなければいけないですが、その分自由にあれこれ作り込めます。初めてのメッセージ送信なので、先ずはお手軽に1のLINE公式アカウント管理アプリから送ってみましょう。
...
すると、あなたがLINEで登録しているプロフィール情報をLINE公式アカウントの管理アプリに提供して良いか、という認証画面が表示されます。問題なければ［許可する］をタップします。もしここで許可した情報提供を、後で取り消したくなった場合は、LINEの［設定］から［アカウント］を開いて、［連動アプリ］で確認したり、取り消したりできます。（@<img>{lineoa-app-3}）

//image[lineoa-app-3][［許可する］をタップ][scale=0.4]{
//}

出てきたー！さっき作ったLINE公式アカウントが、管理アプリで表示されました！（@<img>{lineoa-app-4}）

//image[lineoa-app-4][管理アプリでLINE公式アカウントが表示された][scale=0.4]{
//}
