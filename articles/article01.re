...
LINE公式アカウントには、企業や店舗だけでなく、芸能人のものも存在します。たとえば地震のときに誰よりも早く「大丈夫？」とメッセージをくれて、彼氏感がすごい、と話題になった芸能人のLINE公式アカウント@<fn>{takeru}もありました。@<fn>{line-at}
...
//footnote[programming][「はじめに」で「プログラム書いて動かしてみようぜ！」と元気よく誘ってはじまった割に、ここからしばらくスマートフォンの画面でぽちぽちする作業が続きます。「俺は開発をしに来たのになんでここで焼きそばを作らされているんだ…」みたいな気持ちになるかもしれませんが、第1章で手作業を体験しておくことで、第2章でチャットボットに何をさせているのかが理解しやすくなるはずです。第1章の間は前向きな気持ちで焼きそばをお楽しみください。]
...
LINE公式アカウントを紹介する画面が表示されたら、［公式アカウントを作成］をタップしてください。先ずはLINE公式アカウントのプロフィール画像とアカウント名を設定します。アカウント名と言われてもピンとこないかもしれませんが、もしあなたが技術書典に出展しているなら、サークルの名前とアイコンを設定して、サークルの宣伝をするLINE公式アカウントにしてもいいでしょう。あるいは自分のハンドルネームにしてもいいと思います。このアカウント名はユニークではないため、既存のLINE公式アカウントとかぶる「テストアカウント」のような名称であっても付けられます。@<fn>{name}（@<img>{start-line-oa-1}）
...
//image[setting-oa][大業種と小業種を選んだら［アカウントを作成］をタップ][scale=1]{
//}
