...
//image[list-of-plans][2023年6月時点での料金プラン][scale=1]{
//}
...
先ずはLINEを開いて、下に並んでいるホーム、トーク、VOOM、ニュース、ウォレットという5つのタブのうち、中央にある［VOOM］タブをタップしてください。（@<img>{select-voom}）

//image[select-voom][下部の中央にある［VOOM］タブをタップ][scale=0.8]{
//}
...
//image[mail-address-regist-done][メールアドレスを登録するとLINEにメッセージが届く][scale=0.4]{
//}

//footnote[mail][登録したメールアドレスを後で変更したくなったら、下に並んでいる［ホーム］タブから右上の歯車アイコンをタップして［設定］を開き、［アカウント］の［メールアドレス］で変更できます。]

それでは改めて［VOOM］タブを開いて、右上にある人型のアイコンから、［公式アカウント作成・連携］というボタンをタップしてください。すると、LINE公式アカウントを紹介する画面が表示されますので、続けて［公式アカウントを作成］をタップします。（@<img>{start-line-oa}）

//image[start-line-oa][［公式アカウントを作成］をタップ][scale=0.4]{
//}

先ずはLINE公式アカウントのプロフィール画像とアカウント名を設定します。アカウント名と言われてもピンとこないかもしれませんが、もしあなたが技術書典に出展しているサークルさんなら、サークルの名前とアイコンを設定して、サークルの宣伝をするLINE公式アカウントにしてもいいでしょう。あるいは自分のハンドルネームにしてもいいと思います。このアカウント名はユニークではないため、既存のLINE公式アカウントとかぶる「テストアカウント」のような名称であっても付けられます。この後、実際にメッセージを送ってみたときにうれしくなるので、何かしら愛着の持てる名前を付けておくことをお勧めしますが、プロフィール画像もアカウント名も後から変更が可能ですので、悩みすぎずに取りあえずの画像や名前を設定しても構いません。（@<img>{setting-oa-1}）
...
//image[setting-oa-2][LINE公式アカウントの使い道に応じて適した大業種と小業種を選ぼう][scale=0.4]{
//}
...
ここからはいくつか同意の必要なページが出てきます。先ず［情報利用に関する同意について］です。こちらもよく読んで、自分が同意できる内容であることを確認してから［同意する］をタップします。（@<img>{consent-of-use}）

//image[consent-of-use][よく読んでから［同意する］をタップ][scale=0.4]{
//}

続いては［ZHDグループへの情報提供に関する個別規約への同意について］です。これもきちんと読んでから、同意できる内容であれば［同意する］をタップします。（@<img>{zhd}）
...
LINE公式アカウントから友だちにメッセージを送る方法は、以下の3つ@<fn>{cannot}があります。
...
自由度は1＜2＜3だと思ってください。1はスマホさえあれば簡単にメッセージが送れますが、細かい設定はできません。2は、1に比べるともう少し凝ったメッセージが作れますが、やはり細かい部分で自由度に制限があります。そして3だと自分でコードを書かなければいけないですが、その分かなり自由にメッセージを作り込めます。
...
メッセージの見え方を確認して問題なければ、［次へ］をタップします。配信先が［すべての友だち］になっていることを確認し、［配信］をタップしてメッセージを送ってみましょう。［このメッセージはすべての友だちにすぐに配信されます。配信しますか？］という確認が表示されました。さてここで本当に［すべての友だち］に送ったら、LINEの友だち全員に新刊の購入を迫る変なメッセージが届いてしまわないか、ちょっと不安になりますね。（@<img>{send-message-via-app-3}）
...
では自分のLINEに戻って、メッセージが届いたか確認してみましょう。友だちみんなには届かなくても、少なくともLINE公式アカウントの創造主である自分にはメッセージが届いているかな、と思いましたが、残念ながら何も届いていません。メッセージを送信した履歴も残っていません。（@<img>{no-message-received}）
...
先ほど作ったLINE公式アカウントの友だち追加画面が表示されました。［友だち 0］［フォロワー 1］と書いてあります。このフォロワーというのはLINE VOOMのフォロワーのことです。さっきLINE公式アカウントを作ったときに、自動的に自分で自分をフォローしていたので、この［フォロワー 1］は自分自身です。知らない誰かではありません。
...
Keepメモに貼った友だち追加用のURLをうっかりTwitterなどに書いてしまうと、誰でもあなたのLINE公式アカウントと友だちになれてしまいます。自分しかメッセージを受け取っていないつもりで色々テストしていたら、実は自分以外の人もひっそり見ていた、ということにならないよう、友だち追加のURLの取り扱いには注意しましょう。
...
自分以外とは友だちになっていない状態に戻したいのであれば、友だち追加されてしまったLINE公式アカウントは削除して、新しいLINE公式アカウントを作り直しましょう。LINE公式アカウントは、LINE Official Account Manager、またはLINE公式アカウント管理アプリの［アカウント設定］から、［アカウントを削除］で削除できます@<fn>{delete}。LINE公式アカウントは、一度削除してしまうと復活させることはできないので注意してください。
...
ターゲットリーチとは、要は「送信対象となる友だち」の総数のことのようです。友だちできた…ひとりできた…自分だけど…。鏡に向かって「私たち今日から友だちだね…！」と言っている気分になってきましたが、例え相手が自分自身だろうが友だちは友だちです！メッセージを送ってみましょう。先ほどと同じように［メッセージを配信する］をタップします。
...
［トークルーム］では、こんな感じの見た目になるようです。さっきより要求が強めなメッセージが準備できました。（@<img>{preview-message-again}）

//image[preview-message-again][トークルームやトークリストでの見え方が送信前に確認できる][scale=0.6]{
//}
...
ピロン！LINEの通知が来ました！新刊の購入をお願いするメッセージが届いたようです。やったね！これで「LINE公式アカウントから友だちにメッセージを送る」というアチーブメントを達成しました。（@<img>{got-a-message}）
...
メッセージの通数は送信対象となった友だちの人数でカウントされますので、このように3つのメッセージオブジェクトをまとめて送った場合でも、カウントは1通となります。もしテキストを送る、画像を送る、スタンプを送る、というようにメッセージを3回に分けて送るとカウントは3通になります。ちなみに1つのメッセージにつきメッセージオブジェクトはLINE Official Account Managerや管理アプリでは最大3つ、Messaging APIでは最大5つまで指定できます。

メッセージは友だちとLINE公式アカウントの1対1のトークだけでなく、何人もの友だちが参加しているグループトークに対しても送信可能です。前述のとおり、メッセージの通数は送信対象となった友だちの人数でカウントされますので、友だち3人とLINE公式アカウントが参加しているグループトークに、LINE公式アカウントからメッセージを送った場合、メッセージの通数は3通としてカウントされます。
...
//image[line-official-account-and-line-account][左がLINE公式アカウントで、右が普通のLINEアカウント][scale=0.8]{
//}
...
普通のLINEアカウントとLINE公式アカウントには、このような差があります。
