...
//image[richmenu-example][リッチメニューの画像][scale=0.4]{
//}
...
//image[add-richmenu-playground][［メッセージ送信］をタップするとメッセージが送信されて返信が届いた][scale=0.3]{
//}
...
//image[richmenu-template][リッチメニューのテンプレート画像がダウンロードできる][scale=0.8]{
//}
...
限定された機能で構わないのでラクにリッチメニューが設定したいならLINE Official Account Managerを使って、こだわったリッチメニューが設定したいならMessaging APIを使って自分だけの管理画面を作るのがよいでしょう。
...
意外と知らない人が多いのですが、実はスマートフォンで使っているLINEと同じアカウントで、PC版のLINEも使うことができます。Messaging APIの動作検証をするため、LINE公式アカウントにメッセージを送る場合は、PC版のLINEで作業した方がやりやすいです。ただしリッチメニューなど、PC版のLINEではサポートされていない機能も一部ありますので注意してください。
