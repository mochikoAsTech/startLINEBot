...
ナローキャストメッセージ@<fn>{audience}を使うと、性別や年齢、地域、友だちになってからの期間といった属性情報を指定してメッセージを送ることが可能です。ただし属性情報を指定したメッセージ送信には、LINE公式アカウントのターゲットリーチが100人以上で、かつ属性情報を指定して絞り込んだ送信対象も50人以上であること、という制限事項があります。そもそも友だちが少なかったり、条件を絞り込みすぎて送信対象者が少なかったりすると、属性情報を指定したメッセージ送信はできませんので注意してください。@<fn>{limit}
...
たとえばブロードキャストメッセージで新商品予告のメッセージを全員に送って、そのメッセージのURLを開いたユーザーを「新商品に興味のあるオーディエンス」に入れておき、新商品の発売当日はそのオーディエンスだけを対象にして店頭イベントの告知を送る、というようなこともできます。
...
//image[richmenu][リッチメニュー][scale=0.6]{
//}

このリッチメニューは、一番下の「新刊を読む！▼」と書いてあるバーをタップすることで開いたり閉じたりできます。このバーのテキストも、デフォルトの「メニュー」から「新刊を読む！」「メニューはこちら」「お問い合わせの入力はこちら」「ほっとするブレイクタイムを」「会員証」「お役立ち情報はこちら」「人気ランキングをチェック！」「こちらもチェック」「荷物の追跡・再配達・集荷受付」といった様々なテキストに変更できます。
...
ただしLINE Official Account Managerでリッチメニューを作る場合、大なら7種類、小なら5種類ある特定のサイズや形からしか選べません。また全員に同じリッチメニューを出す機能しかないため、会員証を持っている人にだけ会員向けのリッチメニューを出す、といった出し分けもできません。Messaging APIであれば、縦横のサイズや、それぞれの領域も自由に設定でき、ユーザーごとに別々のリッチメニューを表示させることも可能ですし、タブ切り替えのようなリッチメニューを作ることもできます。
