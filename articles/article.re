...
個別対応のコストが一切要らなくなるものの、ユーザー起点のコミュニケーションはできなくなるので、一方的な宣伝をするだけの場所になります。
...
3つめは応答メッセージとAI応答メッセージで自動返信する、という方法です。

2つめと3つめの方法は併用することも可能です。LINE Official Account Managerや管理アプリで営業時間を設定しておくことで、営業時間内は中の人が手打ちのチャットで応答して、営業時間外は自動応答のメッセージに任せる、というような運用ができます。

AI応答メッセージは、このアカウントに送信されたLINEユーザーからの問い合わせに対し、管理ツールで設定した内容に従ってチャットボットが自動で応答する機能です。
...
でも片方が話したいときだけ一方的に話すのでは、それは対話とは言えません。ユーザーが友だち追加してくれたときや、ユーザーが話しかけてきたとき、ユーザーがリッチメニューをタップしたときなど、ユーザー起点のコミュニケーションにきちんと反応できるようになってこそのボットと言えます。
