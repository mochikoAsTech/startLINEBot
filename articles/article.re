...
//image[line-official-account-and-line-account][LINE公式アカウント][scale=0.4]{
//}
...
たくさんの友だちにメッセージを継続して送ろうとすると、有料のプランを使う必要がありますが、試しにちょっと開発してみる程度なら無料のフリープランで問題ありません。無料メッセージの通数を使い切れば、次の月初までメッセージが送れなくなるだけで、自分で有料のライトプランやスタンダードプランに申し込まない限りは、メッセージの使いすぎで勝手に課金が始まることはありませんので安心してください。
...
LINEを開いて下に並んでいるホーム、トーク、VOOM、ニュース、ウォレットという5つのメニューのうち、中央にある［VOOM］をタップしてください。（@<img>{select-voom}）

//image[select-voom][下部の中央にある［VOOM］をタップ][scale=0.4]{
//}

VOOMとやらがなんなのかということはいまは考えてはいけません。きっとショート動画のプラットフォーム的な何かです。VOOMを開いたら、右上にある人型のアイコンをタップします。（@<img>{select-icon}）
...
このとき、もし［公式アカウントを作成するか、既存アカウントを連携するには、メールアドレスを登録してください］と表示されたら、あなたはまだLINEアカウントにメールアドレスを登録していないようです。そのまま［登録］をタップします。（@<img>{regist-mail-address}）
...
//image[mail-address-check][メールアドレス登録確認メールが届く][scale=0.4]{
//}
...
//image[mail-address-regist-done][4桁の認証番号をLINEで入力][scale=0.4]{
//}

//footnote[mail][登録したメールアドレスを変更したい場合は、下に並んでいる［ホーム］から右上の歯車アイコンをタップして［設定］を開き、［アカウント］の［メールアドレス］で変更できます。]

それでは改めて、VOOMの右上にある人型のアイコンから、［公式アカウント作成・連携］というボタンをタップしてください。
